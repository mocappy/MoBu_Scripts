...
def getBaseLayerPerFrameValueChange(scene_object ):
    """
    Get Amount of Change Between Key Frames on Object
    Return values for all animation nodes
    Lcl Translation
    Lcl Rotation
    Lcl Scaling    
    """
                ...
                if transAxis.Name in transNodeName_transAxisName_keyFrame_differenceValue[transNode.Name]:
                    ...
                    for keyFrame in sorted(transNodeName_transAxisName_keyFrame_differenceValue[transNode.Name][transAxis.Name].keys()):
                        ...
                        if valueChange > 0.0:
                            ...
                            percentChange = 0.0
                        # Calculate Layer 1 Change percentage
                        incremetnValue = (percentChange * endValue) / 100.0
                        # Calc Delta Value
                        deltaValue = deltaValue + incremetnValue
                        # Add Delta Value Key to FCurve
                        transAxis.FCurve.KeyAdd( FBTime(0, 0, 0, keyFrame), deltaValue)
                        print deltaValue
                        # if not round(deltaValue, 2) == round(endValue, 2):
                        #    print round(deltaValue, 2), round(endValue, 2)
        
def fixFootSlide(characterName):
    if not FBSystem().CurrentTake.GetLayerCount() > 1:
        print "ADJUST BLEND ERROR:\n - Only Single '{}' Layer Found".format(FBSystem().CurrentTake.GetLayer(0).Name)
    else:
        currentChar = characterAction.findCurrentCharacter(characterName)
        for lEffectorId in FBEffectorId.values.values():
            ...
            if characterEffector:
                print characterEffector.Name
                adjustBlend( characterEffector) 
                break

fixFootSlide('Character')